...
if(!require('devtools')) install.packages("network")
...
![hemibrain_opns_mb](https://raw.githubusercontent.com/natverse/neuprintr/master/inst/images/hemibrain_opns_mb.png)

So, previously ~35 OPN cells were known (variable between brains) that fall into ~22 types [(Aso et al. 2014a,](https://www.ncbi.nlm.nih.gov/pubmed/25535793) [Aso et al. 2014b,](https://www.ncbi.nlm.nih.gov/pubmed/25535794) [Takemura et al. 2017)](https://www.ncbi.nlm.nih.gov/pubmed/28718765). Here, according to the naming in the hemibrain project, there are putative 75 OPN cells that can be broken down into ~35 putative types.
...
# And highlight glomerulus alpha prime 3
ca.mesh = neuprint_ROI_mesh(roi = "CA(R)")
lh.mesh = neuprint_ROI_mesh(roi = "LH(R)")
malt.mesh = neuprint_ROI_mesh(roi = "mALT(R)")
plot3d(ca.mesh, add = TRUE, alpha = 0.5, col = lacroix[["pink"]])
plot3d(lh.mesh, add = TRUE, alpha = 0.5, col = lacroix[["green"]])
...
nopen3d(userMatrix = structure(c(0.98370349407196, -0.104569993913174, 
                                 ...
                                 0.0147849693894386, 0, 0, 0, 0, 1), .Dim = c(4L, 4L)), zoom = 0.644609212875366, 
        windowRect = c(20L, 65L, 1191L, 843L))
plot3d(al.mesh, add = TRUE, alpha = 0.1, col = "grey")
...
Then we will follow up on their connectivity with other neurons in the brain, and focus on a 'teaching' dopaminergic input onto a OPN type, OPN-a'3, which is thought to allow this OPN to act as a novelty detector [(Hattori et al. 2017)](https://www.ncbi.nlm.nih.gov/pubmed/28502772).

![opn_paths_to_PPL1ap3](https://raw.githubusercontent.com/natverse/neuprintr/master/inst/images/opn_paths_to_PPL1ap3.png)
...
opn.adj.comp = t(apply(t(opn.adj.comp), 2, function(x) tapply(x, colnames(opn.adj.comp), mean, na.rm = TRUE)))
opn.adj.comp = apply(opn.adj.comp, 2, function(x) tapply(x, rownames(opn.adj.comp), mean, na.rm = TRUE))
...
### The cognate DAN for OPNs-a'3, PPL1-a'3, gets 49 (!) of its inputs from OPNs-a'3
